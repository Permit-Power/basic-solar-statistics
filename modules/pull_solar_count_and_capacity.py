...
def _download_non_net_raw(year: int) -> pd.DataFrame:
    ...
    URL patterns:
    - current calendar year: xls/non_netmetering{year}.xlsx
    - prior years:           archive/xls/non_netmetering{year}.xlsx (with underscore fallback)
    """
    candidates: List[str] = []
...
def _download_net_raw(year: int) -> pd.DataFrame:
    ...
    URL patterns:
    - current calendar year: xls/net_metering{year}.xlsx
    - prior years:           archive/xls/net_metering{year}.xlsx (with underscore fallback)
    """
    candidates: List[str] = []
    current_year = datetime.date.today().year
