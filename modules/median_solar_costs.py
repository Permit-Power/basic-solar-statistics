...
# ---------------------------------------------------------------------------
# LOCAL PIPELINE (no download required)
# ---------------------------------------------------------------------------


def load_median_solar_costs_and_size(
    fred_api_key: str,
    lbnl_path: str = "../data/TTS_LBNL_public_file_29-Sep-2025_all.csv",
    year: int = 2024,
    start_year: int = 2000,
) -> pd.DataFrame:
    """
    Load median solar costs and system size for a single year from a local LBNL CSV.

    Parameters
    ----------
    fred_api_key : str
        FRED API key for inflation adjustment.
    lbnl_path : str
        Path to the local LBNL Tracking the Sun CSV.
    year : int
        Installation year to return. Default: 2024.
    start_year : int
        Earliest CPI year to fetch from FRED. Default: 2000.

    Returns
    -------
    pd.DataFrame
        Columns: state, median_price_per_kw, median_system_size_kw
    """
    lbnl = pd.read_csv(lbnl_path)
    inflation = build_inflation_multipliers(fred_api_key, start_year=start_year)
    all_years = compute_median_solar_costs_and_size(lbnl, inflation, min_year=year)
    result = all_years[all_years["year"] == year][["state", "median_price_per_kw", "median_system_size_kw"]]
